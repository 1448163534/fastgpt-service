  ...
  const token = jwt.sign(
    {
      ...
      exp: Math.floor(Date.now() / 1000) + 60 * 60 * 24 * 70
    },
    key
  );
    const decoded = jwt.verify(token, key) as JwtPayload;
    addLog.error('Token created successfully:');
    addLog.error('Issued at:', new Date(decoded.iat * 1000));
    addLog.error('Expires at:', new Date(decoded.exp * 1000));
  return token;
}
...
export function authJWT(token: string) {
  return new Promise<{
    ...
    jwt.verify(token, key, (err, decoded: any) => {
      ...
      addLog.error('Token verification time:', new Date());
      addLog.error('Token expiration time:', new Date(decoded.exp * 1000));
      resolve({
...
export async function parseHeaderCert({
  ...
  // from tauthorization, root-pending
  async function parseTAuthorization(tauthorization?: string) {
    if (!tauthorization) {
       return Promise.reject(ERROR_ENUM.unAuthorization);
    }
    const auth = tauthorization.split(' ')[1];
    if (!auth) {
       return Promise.reject(ERROR_ENUM.unAuthorization);
    }
    return await authJWT(auth);
  }
  ...
  // root-pending
  const tauthorization = req.headers['t-authorization'] as string | undefined;
  ...
  const { uid, teamId, tmbId, appId, openApiKey, authType, isRoot, sourceName } =
    await (async () => {
      ...
      // root-pending, 不使用cookie
      if (authToken && tauthorization && process.env.USE_COOKIE === 'false') {
          const res = await parseTAuthorization(tauthorization);
          return {
              uid: res.userId,
              teamId: res.teamId,
              tmbId: res.tmbId,
      ...
      if (authToken && (token || cookie || authorization)) {
        // user token(from fastgpt web)
        const res = await authCookieToken(cookie, token || authorization?.split(' ')[1]);
        return {
          uid: res.userId,
          teamId: res.teamId,
          tmbId: res.tmbId,
